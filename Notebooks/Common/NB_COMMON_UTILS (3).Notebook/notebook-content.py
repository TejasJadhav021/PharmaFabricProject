...
def optimize_table(
    ...
    if zorder_columns:
        ...
        print(f"OPTIMIZE + ZORDER BY ({zorder_clause}) completed on {table_name}")

    else:

        spark.sql(f"OPTIMIZE {table_name}")

        print(f"OPTIMIZE completed on {table_name}")
...
def vacuum_table(
    ...
    print(f"VACUUM completed on {table_name} (retention: {retention_hours}h)")


def set_delta_table_properties(
    table_name: str,
    properties: dict
):
    ...
    print(f"Table properties set on {table_name} : {properties}")
...
def write_audit_log(
    ...
    """
    Appends one row to the pipeline audit table (creates it on
    first use if it doesn't exist yet). Call this once per
    notebook run - on success AND on failure - so every run has a
    traceable record: what ran, how many rows, how long, and (on
    failure) why.
    """
    ...
    print(f"Audit log written -> {audit_table} [{status}]")
